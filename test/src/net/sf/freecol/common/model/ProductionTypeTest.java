...
public class ProductionTypeTest extends FreeColTestCase {
    ...
    private static final GoodsType gunpowder
            = s.getGoodsType("model.goods.gunpowder");
    ...
    private static final TileType caves
            = s.getTileType("model.tile.caves");
    ...
    public void testCaves() {
        testProduction(inputs, outputs,
                caves.getAvailableProductionTypes(true));

        outputs.put(ore, 4);
        outputs.put(gunpowder, 5);

        testProduction(inputs, outputs,
                caves.getAvailableProductionTypes(false));

        assertEquals(0, caves.getPotentialProduction(grain, null));
        assertEquals(0, getGenericPotential(caves, grain));
    }
